...
//            ps.setString(4,customer.phone);
    ...
    public void select() {
        connection = OracleUtil.getCon();
        String sql = "select * from HIS_ZN.FIN_IPR_INMAININFO  where  INPATIENT_NO='ZY010000755802' ";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                Object id = rs.getObject("INPATIENT_NO");
                Object date1 = rs.getObject("BIRTHDAY");

                System.out.println("id:" + id + "date1:" + date1);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
